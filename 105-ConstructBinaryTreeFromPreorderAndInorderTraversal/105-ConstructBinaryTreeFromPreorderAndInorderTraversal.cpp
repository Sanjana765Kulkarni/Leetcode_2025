// Last updated: 01/08/2026, 23:23:54
/**
...
public:
    TreeNode* build(vector<int>& preorder, int preStart, int preEnd, vector<int>& inorder, int inStart, int inEnd, map<int,int>& mp)
    {
        ...
        root->left = build(preorder,preStart+1,preStart+ NumsLeft,inorder, inStart, index +1,mp);
        root->right = build(preorder,preStart+NumsLeft+1,preEnd,inorder, index+1, inEnd,mp);
